...
export type StampShape = "plus" | "heart" | "diamond" | "x";
...
export const DrawingCanvas = forwardRef<DrawingCanvasHandle, Props>(
  function DrawingCanvas(
    { tool, color, symmetry, stampShape, onBeforeStroke },
    ref,
  ) {
    ...
    useEffect(() => {
      ...
      const onDown = (e: MouseEvent | TouchEvent) => {
        ...
        switch (tool) {
          case "pencil":
            withMirror(pos.x, (px) =>
              ctx.fillRect(Math.round(px), Math.round(pos.y), PX, PX),
            );
            break;
          ...
          case "eraser":
            withMirror(pos.x, (px) =>
              ctx.clearRect(
                Math.round(px) - ERASER_H,
                Math.round(pos.y) - ERASER_H,
                ERASER,
                ERASER,
              ),
            );
          ...
          case "spray":
            ...
            sprayTimer.current = setInterval(() => {
              ...
              ctx.fillStyle = color;
              withMirror(currentPos.current.x, (px) =>
                sprayAt(ctx, px, currentPos.current!.y),
              );
            }, 40);
            break;
        }
      ...
      const onMove = (e: MouseEvent | TouchEvent) => {
        ...
        if (tool === "dots" && lastPos.current) {
          ...
          dotsCounter.current = bresenhamDotted(
            lp.x,
            lp.y,
            pos.x,
            pos.y,
          ...
          if (symmetry)
            bresenhamDotted(mx(lp.x), lp.y, mx(pos.x), pos.y, c0, plot);
          lastPos.current = pos;
        }
      ...
      const onUp = () => {
        ...
        if (strokePath.current.length > 1) {
          ...
          console.log(
            `%cCurve captured — ${rounded.length} points (x/y as 0–1 viewport fractions):`,
            "font-weight:bold",
          );
